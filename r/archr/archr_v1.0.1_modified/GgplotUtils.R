  ...
  if (is.null(color) & !colorDensity) {
    ...
    if (discrete) {
        ...
        if (!is.null(pal)) {
	      ...
	      if (colorLimits[1] == 0) {
              	p <- p + scale_colour_gradientn(colors = pal, limits=colorLimits, breaks=seq(0.5, colorLimits[2], by=0.5), na.value = "lightgrey", oob = scales::squish)
	      } else {
              	p <- p + scale_colour_gradientn(colors = pal, limits=colorLimits, na.value = "lightgrey", oob = scales::squish)
	      }
            }else{
              #p <- p + scale_colour_gradientn(colors = pal, na.value = "lightgrey")
              p <- p + scale_colour_gradientn(colors = pal, na.value = "lightgrey", oob = scales::squish)
            }
        } else {
          if(!is.null(colorLimits)){
            #p <- p + scale_colour_gradientn(colors = paletteContinuous(set = continuousSet), limits=colorLimits, na.value = "lightgrey")
	    if (colorLimits[1] == 0) {
		p <- p + scale_colour_gradientn(colors = paletteContinuous(set = continuousSet), limits=colorLimits, breaks=seq(0.5, colorLimits[2], by=0.5), na.value = "lightgrey", oob = scales::squish)
	    } else {
		p <- p + scale_colour_gradientn(colors = paletteContinuous(set = continuousSet), limits=colorLimits, na.value = "lightgrey", oob = scales::squish)
	    }
          } else {
            #p <- p + scale_colour_gradientn(colors = paletteContinuous(set = continuousSet), na.value = "lightgrey")
            p <- p + scale_colour_gradientn(colors = paletteContinuous(set = continuousSet), na.value = "lightgrey", oob = scales::squish)
          }
        }
        # end of modification
    }
