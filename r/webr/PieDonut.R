...
# modified by H. Kim
# copied: 2019, July
# lasted modified: 2020, April
#
# copied from:
# https://github.com/cardiomoon/webr/blob/master/R/PieDonut.R
#
# add arguments:
...
PieDonut=function(data,mapping,
        ...
        if((labelposition==1)&(is.null(donuts))){
                p1<-p1+ geom_segment(aes_string(x="segx",y="segy",
                   ...
                   geom_text(aes_string(x="segxend",y="segyend",label="label",hjust="hjust",vjust="vjust"),size=pieLabelSize,data=df,family=family,lineheight=lineheight) # . Kim added lineheight

        } else if((labelposition==2)&(is.null(donuts))){
                p1<-p1+ geom_segment(aes_string(x="segx",y="segy",
                    xend="segxend",yend="segyend"),data=df[df$ratio<labelpositionThreshold,])+
                    geom_text(aes_string(x="segxend",y="segyend",label="label",hjust="hjust",vjust="vjust"),size=pieLabelSize,data=df[df$ratio<labelpositionThreshold,],family=family,lineheight=lineheight)+ # H. Kim added lineheight
                    geom_text(aes_string(x="labelx",y="labely",label="label"),size=pieLabelSize,data=df[df$ratio>=labelpositionThreshold,],family=family,lineheight=lineheight) # H. Kim added lineheight

        } else{
                p1 <-p1+geom_text(aes_string(x="labelx",y="labely",label="label"),size=pieLabelSize,data=df,family=family,lineheight=lineheight) # . Kim added lineheight
        }

        if(showPieName) {
          # begin modification by H. Kim
          #p1<-p1+annotate("text",x=0,y=0,label=pie,size=titlesize,family=family)
          if (is.null(pieName)) pieName <- pies
          p1<-p1+annotate("text",x=0,y=0,label=pieName,size=titlesize,family=family)
          # end modification by H. Kim
        }
        p1<-p1+theme(text=element_text(family=family))
        # begin addition by H. Kim
        p1<-p1+labs(x='', y='') + theme_bw() +
