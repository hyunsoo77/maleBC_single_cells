#
# perfrom_p2g_permutation_tests-firstquart.R
...
#saveRDS(store.prop, sprintf("%s/store_null_knn_proportions-firstquart.rds", dir_rds))

pdf(sprintf("%s/hist_patient_purity_per_cel_aggregate-firstquart.pdf", dir_pdf), width = 5,height = 3.5)
...
p2g.df.null <- as.data.frame(p2geneDF)
p2g.df.null <- p2g.df.null[complete.cases(p2g.df.null),]
#saveRDS(p2g.df.null, sprintf("%s/pg2.df.null-firstquart.rds", dir_rds))


pdf(sprintf("%s/hist_p2g_correlation-firstquart.pdf", dir_pdf), width = 5,height = 3.5)
hist(p2g.df.null$Correlation, col = "lightblue", main = "Histogram of null peak-to-gene correlations", xlab = "Correlation")
dev.off()

pdf(sprintf("%s/hist_p2g_pval-firstquart.pdf", dir_pdf), width = 5,height = 3.5)
...
#saveRDS(store, sprintf("%s/store_null_tests-firstquart.rds", dir_rds))
